...
export default function FixedNavbar() {
  ...
  const { isAuthenticated, logout, loading, user } = useAuthClient();
    ...
    return (
      <>
        <div className="hidden md:flex items-center space-x-8">
          ...
          <Link href="/explore" className="text-sm text-zinc-300 hover:text-white">Explorer</Link>
        </div>

        <div className="flex items-center gap-3">
          <Link href="/login" className="text-sm text-zinc-300 hover:text-white">Connexion</Link>
          <Link href="/get-started" className="hidden sm:inline-flex items-center px-4 py-2 rounded-full bg-gradient-to-br from-blue-600 to-violet-600 text-white hover:from-blue-700 hover:to-violet-700 shadow-md">S'inscrire</Link>
        </div>
      </>
    );
  }
  ...
  function PrivateLinks() {
    return (
      <>
        <div className="hidden md:flex items-center space-x-8">
          <Link href="/dashboard" className="text-sm text-zinc-300 hover:text-white">Tableau de bord</Link>
          <Link href="/services" className="text-sm text-zinc-300 hover:text-white">Services</Link>
          <Link href="/explore" className="text-sm text-zinc-300 hover:text-white">Explorer</Link>
        </div>
    ...
    <nav className="fixed top-0 left-0 w-full z-[100] bg-[#0f1724]/80 backdrop-blur-md border-b border-white/10">
      <div className="max-w-7xl mx-auto px-6">
        <div className="flex items-center justify-between h-16">
          <Link href="/" className="flex items-center gap-3 group text-zinc-50">
            <Logo className="w-9 h-9" />
            <span className="font-semibold text-lg transition-colors group-hover:text-blue-400">Nexus</span>
          </Link>

          {loading ? null : (isAuthenticated ? <PrivateLinks /> : <PublicLinks />)}
        ...
        <div className="md:hidden bg-background/80 backdrop-blur-sm">
          <div className="max-w-7xl mx-auto px-6 py-3 space-y-2">
            {loading ? null : (isAuthenticated ? (
              <>
                <Link href="/dashboard" className="block px-3 py-2 rounded-md text-base font-medium text-muted-foreground">Tableau de bord</Link>
                <Link href="/services" className="block px-3 py-2 rounded-md text-base font-medium text-muted-foreground">Services</Link>
                <Link href="/explore" className="block px-3 py-2 rounded-md text-base font-medium text-muted-foreground">Explorer</Link>
                {/* Se déconnecter button removed */}
              </>
            ) : (
              <>
                <Link href="/services" className="block px-3 py-2 rounded-md text-base font-medium text-muted-foreground">Services</Link>
                ...
                <div className="pt-2 border-t border-border">
                  <Link href="/login" className="block px-3 py-2 rounded-md text-base font-medium text-muted-foreground">Connexion</Link>
                  <Link href="/get-started" className="block mt-2 px-3 py-2 rounded-full bg-gradient-to-br from-blue-600 to-violet-600 text-white text-center">S'inscrire</Link>
                </div>
              </>
            ))}
          </div>
