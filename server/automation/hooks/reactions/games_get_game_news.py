...
@register_reaction("get_game_news")
def executor(area, context=None):
    """
    Wrapper for GamesProvider.get_game_news
    """
    try:
        provider = ProviderRegistry.get_provider("games")
        config = area.config_reaction or {}
        
        # Template parameter replacement (e.g. game_id from context)
        if context:
        ...
        return provider.get_game_news(config, context)
